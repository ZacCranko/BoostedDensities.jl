...
begin
    using BoostedDensities, Distributions, Flux, JLD2, NPZ
    global VERBOSE = 4      # display how we're doing
    ...
    include(joinpath(Pkg.dir("BoostedDensities"), "experiments", "kernel_density_estimation.jl"))
end

architecture_comparison() = begin
    ...
    models = [ # classifier architectures
        ...
        Chain(Dense(d, 20, Flux.selu), Dense(20, 20, Flux.selu), Dense(20, 1))
    ]

    results = Array{Any}(3, length(models), RUNS)
    for i in Base.OneTo(RUNS)
        VERBOSE == 1 && info("worker $(myid()): architecture comparison: $i of $RUNS")
        srand(i)
        means = rand(MvNormal(d, 4), 8)
        rp    = GaussianMixture(mapslices(x->MvNormal(x, 1), means, 1) |> vec) # randomly arranged Gaussians
        num_p, num_q = 500, 500
        ...
        for (j,m) in enumerate(models)
            ...
            q, train_history, boosting_history = run_experiment(rp, q₀, num_p, num_q, iter = iter, num_epochs = 1000, model = m,
                                                                run_boosting_metrics = false, verbose = VERBOSE >= 3, optimiser = ADAM, optimise_alpha = true)
...
activation_comparison() = begin
    ...
    for i in Base.OneTo(RUNS)
        ...
        for (j,act) in enumerate(activations)
            ...
            q, train_history, boosting_history = run_experiment(p, q₀, num_p, num_q, iter = iter, alpha = t->0.5, num_epochs = 400, model = m,
                                                                run_boosting_metrics = true, verbose = VERBOSE >= 3, optimiser = ADAM)
            results[:,j,i] = [q, train_history, boosting_history]
        end
    end
...
kde_comparison() = begin
    d = 2
    conditions = ["deep network", "scott/silverman", "cross_validated"]
    m = length(conditions)
    ...
    for i in Base.OneTo(RUNS)
        ...
        q, train_history, boosting_history = run_experiment(rp, q₀, p_samps, q_samps, train_p_samps, train_q_samps, test_p_samps, test_q_samps,
                                                            iter = 2, num_epochs = 4_000, batch_size = 50,
        ...
        k = kde(p_samps, cross_validate = false)
        pk_kl  = kl(rp, k)
        pk_nll = -mean(logpdf(k, p_samps))
        VERBOSE >= 3 && info("worker $(myid()): nll $pk_nll")
        results[:,2,i] = [nothing, nothing, Dict(nll => pk_nll, kl => pk_kl)]
        ...
        pk_nll = -mean(logpdf(k, p_samps))
        VERBOSE >= 3 && info("worker $(myid()): nll $pk_nll")
        results[:,3,i] = [nothing, nothing, Dict(nll => pk_nll, kl => pk_kl)]
    end
...
function runall(experiments)
    pmap(experiments) do exprmt
        # exprmnt_results = 
        try
            exprmnt_results = exprmt() # run experiment
        catch error
            throw(error)
        end
...
# runall(map(a->eval(Symbol(a)), ARGS))
